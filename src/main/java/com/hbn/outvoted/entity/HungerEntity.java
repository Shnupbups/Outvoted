...
public class HungerEntity extends CreatureEntity implements IAnimatable {
    ...
    public ILivingEntityData onInitialSpawn(IWorld worldIn, DifficultyInstance difficultyIn, SpawnReason reason, @Nullable ILivingEntityData spawnDataIn, @Nullable CompoundNBT dataTag) {
        int type;
        if (Outvoted.matchesBlock(this.world.getBlockState(this.getPosition().down()).getBlock(), Blocks.SAND)) {
            type = 0;
        } else if (Outvoted.matchesBlock(this.world.getBlockState(this.getPosition().down()).getBlock(), Blocks.RED_SAND)) {
            type = 1;
        } else {
            type = 2;
        }
