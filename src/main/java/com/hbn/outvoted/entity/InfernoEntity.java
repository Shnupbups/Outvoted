...
public class InfernoEntity extends MonsterEntity implements IAnimatedEntity {
    ...
    EntityAnimationController<InfernoEntity> controller = new EntityAnimationController<>(this, "controller", 5, this::animationPredicate);
        ...
        if (!this.world.isRemote) {
            if (source.getImmediateSource() instanceof LivingEntity && this.isInvulnerable()) {
                ...
                if (entity.getHeldItemMainhand().getItem() instanceof AxeItem) {
                    double itemDamage = ((AxeItem) entity.getHeldItemMainhand().getItem()).getAttributeModifiers(EquipmentSlotType.MAINHAND).get(Attributes.ATTACK_DAMAGE).iterator().next().getAmount() + 1;
